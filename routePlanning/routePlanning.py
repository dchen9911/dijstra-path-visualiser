# TO DO: Add path optimisation for 3d somehow and prevent line crossing volume
# think of ways to make it even more efficient
...
class routePlanning:
    ...
    def __init__(self, flyZone, noFlyZones, rawWaypoints = None):
        self.length = LENGTH # hard coded values for size of the grid
        self.width = WIDTH
        self.grid = [[[gridPoint(j,i,h) for j in range(0, WIDTH)] for i in range(0, LENGTH)] for h in range(0, HEIGHT)]
        self.noFlyZones = []
        ...
        for zone in noFlyZones:
            newNoFlyZone = noFlyZone(zone[0:len(zone) - 1], zone[-1][0], zone[-1][1])
            self.noFlyZones.append(newNoFlyZone)
        
        if rawWaypoints == None:
            self.rawWaypoints = self.generateRandomWaypoints(NUMBER_OF_WAYPOINTS)
        else:
            self.rawWaypoints = rawWaypoints
        
        print("waypoints: ")
        print(self.rawWaypoints)

        for point in self.rawWaypoints:
            self.waypoints.append(Point(point[0], point[1], point[2]))

        # preprocess points by going through each polygon and looking at each points within its bounds
        for zoneObj in self.noFlyZones:
            polygonObj = zoneObj.polygonObj
            (minx, miny, maxx, maxy) = polygonObj.bounds
            for x in range (math.floor(minx), math.ceil(maxx) + 1):
                for y in range (math.floor(miny), math.ceil(maxy) + 1):
                    ...
                    if x < 0 or x >= WIDTH or y < 0 or y >= LENGTH:
                        continue
                    point = Point(x,y)
                    if polygonObj.contains(point):
                        for z in zoneObj.inGrid2Ds:
                            self.grid[z][y][x].setNotAccessible()

        
        # do the same for the geofence (assume that it extends up to max height)
    ...
    def planRoute(self, startPoint, endPoint):
        ...
        # start the path finding by finding distance to first two waypoints
        ...
        print("First two waypoints time: " + str((time.time_ns() - timebig)/1000000000))
        self.createMap()
        waypointDistances = {}
        allPaths = {}
        timebig = time.time_ns()
        ...
        # use dictionary allPaths to get the actual path
        prevWaypoint = 1
    ...
    def dijkstraFindPath(self, startPoint, endPoint, pointsInPath, checkLineIntersection = False, searchRange = DEFAULT_SEARCH_RANGE):
        ...
        for k in range(-searchRange, searchRange + 1):
            ...
            for i in range(-searchRange, searchRange + 1):
                ...
                for j in range(-searchRange, searchRange + 1):
                    tmp.append((i ** 2 + (j*LENGTH/WIDTH)**2 + (k * LENGTH/HEIGHT)**2)**0.5)
        ...
        pq = MinHeap()
        pq.insert(self.grid[startPoint[2]][startPoint[1]][startPoint[0]], self.grid[startPoint[2]][startPoint[1]][startPoint[0]].getDistance())
            ...
            for k in range(-searchRange, searchRange + 1):
                ...
                for i in range(-searchRange, searchRange + 1):
                    ...
                    for j in range(- searchRange,  searchRange + 1):
                        ...
                        newDist = currGridPoint.getDistance() + self.gridScanDistances[k + searchRange][i + searchRange][j + searchRange]
                        if newDist < oldDist:
        ...
        for x in range(len(tempPath) - 2, -1, -1):
            totalDistance = totalDistance + (abs(tempPath[x+1][0] - tempPath[x][0])**2 + abs(tempPath[x+1][1] - tempPath[x][1])**2 + abs(tempPath[x+1][2] - tempPath[x][2])**2) ** 0.5  
    ...
    # empircally faster than creating a copy of a clean grid
    def gridReset(self):
        for z in range(0, HEIGHT):
            for x in range(0, WIDTH):
                for y in range(0,LENGTH):
                    point = self.grid[z][y][x]
                    point.unvisit()
    ...
    def createMap(self):
        ...
        for zone in self.noFlyZones:
            ...
            for i in range(0, len(zone.coords)):
                face = [list(zone.coords[i]) + [baseHeight], list(zone.coords[i]) + [topHeight], list(zone.coords[i-1]) + [topHeight], list(zone.coords[i-1]) + [baseHeight]]
                sideFaces.append(face)
            collection = Poly3DCollection([topFace, bottomFace]+sideFaces, linewidths=1, alpha=0.2)
        ...
        ax.scatter(self.finalPath[0][0], self.finalPath[0][1], self.finalPath[0][2], s = 50, c = 'g')
        ax.scatter(self.finalPath[-1][0], self.finalPath[-1][1], self.finalPath[-1][2], s = 50, c = 'g')
    ...
    def generateRandomWaypoints(self,n):
        waypoints = []
        cnt = 0
        while len(waypoints) < n:
            ...
            y = random.randrange(5, LENGTH - 5)
            z = random.randrange(0, HEIGHT)
            if self.grid[z][y][x].isAccessible():
...
def generateRandomNoFlyZones(n, size):
    ...
    for i in range(0,n):
        x = random.randrange(5,WIDTH-5)
        y = random.randrange(5,LENGTH-5 )
        if x < POLYGON_SIZE*2 and y < POLYGON_SIZE*2 or x > WIDTH - POLYGON_SIZE*2 and y > LENGTH - POLYGON_SIZE*2:
            i = i - 1
            continue
        ...
        # some random height information
        zone.append((random.randrange(round(HEIGHT/4),round(2*HEIGHT/3)), random.randrange(round(2*HEIGHT/3),HEIGHT)))
        zones.append(zone)
    return zones
...
if __name__ == "__main__":
    ...
    flyZone = [(0,0),(WIDTH,5), (WIDTH,LENGTH), (5,LENGTH)]
    if args['manual']:
    ...
    print("Initialisation time: " + str(preProcessingTime), end = "\n\n")
    if args['manual']:
        routePlanningObj.planRoute((2,2), (WIDTH - 3, LENGTH - 3))
    else:
        routePlanningObj.planRoute((2,2, round(HEIGHT/2)), (WIDTH - 3, LENGTH - 3, 0))
